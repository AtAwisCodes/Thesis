...
async function sendWarningToUploader(uploaderId, reportId, videoTitle) {
    ...
    window.executeWarning = async (uid, rid, title) => {
        ...
        try {
            ...
            if (!userDoc.exists) {
                hideLoading();
                showErrorModal('Send Failed', 'User not found. Cannot send warning to non-existent user.');
                return;
            }
            ...
            await db.collection('video_reports').doc(rid).update({
                warningSent: true,
                warningSentAt: firebase.firestore.FieldValue.serverTimestamp(),
                warningSentBy: auth.currentUser.email
            });
