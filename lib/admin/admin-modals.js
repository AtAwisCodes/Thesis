...
async function deleteUser(userId, userName, userEmail) {
    ...
    window.executeDelete = async (uid, name) => {
        ...
        try {
            ...
            // Hide all user's videos (matching Dart implementation)
...
async function sendWarningToUploader(uploaderId, reportId, videoTitle) {
    // Validate uploaderId exists (same validation as Dart: check for null/undefined/empty)
    if (!uploaderId || uploaderId === 'undefined' || uploaderId === 'null' || uploaderId === '') {
        showErrorModal('Send Warning Failed', 'Uploader ID not found. Cannot send warning to unknown user.');
        return;
    }
    
    const warningModalHtml = `
        <div class="modal-overlay" id="warningModal" onclick="closeWarningModal(event)" style="z-index: 1100; background: rgba(0, 0, 0, 0.7);">
            <div class="modal" onclick="event.stopPropagation()">
                <div class="modal-header">
                    ...
                    <button class="icon-button" onclick="closeWarningModal()">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/>
                        </svg>
                    </button>
                ...
                <div class="modal-footer">
                    <button class="btn btn-cancel" onclick="closeWarningModal()">Cancel</button>
    ...
    window.executeWarning = async (uid, rid, title) => {
        ...
        try {
            // Get the actual videoId from the current report
            const reportDoc = await db.collection('video_reports').doc(rid).get();
            const reportData = reportDoc.data();
            const actualVideoId = reportData ? reportData.videoId : '';
            
            // Create warning notification (matching Dart implementation exactly)
            await db.collection('count').doc(uid).collection('notifications').add({
                type: 'warning',
                ...
                videoTitle: title,
                reportId: rid,
                fromUserId: auth.currentUser.uid || 'admin',
                fromUserName: 'Admin',
                fromUserAvatar: '',
                isRead: false,
                createdAt: firebase.firestore.FieldValue.serverTimestamp()
            ...
            hideLoading();
            showSuccessModal('Warning Sent', 'Warning sent successfully');
        } catch (error) {
            console.error('Send warning error:', error);
            hideLoading();
            showErrorModal('Send Failed', error.message || 'An error occurred while sending the warning.');
        }
    };
}
...
async function deleteVideoFromReport(videoId, reportId) {
    // Validate videoId exists (same validation as Dart: check for null/undefined/empty)
    if (!videoId || videoId === 'undefined' || videoId === 'null' || videoId === '') {
        showErrorModal('Delete Failed', 'Video ID not found. The video may have already been deleted.');
        return;
    }
    ...
    showConfirmModal(
        ...
        async () => {
            showLoading();
            try {
                // Check if video document exists (matching Dart implementation)
                const videoDoc = await db.collection('videos').doc(videoId).get();
                if (!videoDoc.exists) {
                    hideLoading();
                    showErrorModal('Delete Failed', 'Video not found in database. It may have already been deleted.');
                    return;
                }
                
                // Soft delete: Mark video as deleted (matching Dart implementation)
                ...
                closeReportDetail();
                hideLoading();
                showSuccessModal('Video Deleted', 'Video deleted successfully. Can be restored if needed.');
            } catch (error) {
                console.error('Delete video error:', error);
                hideLoading();
                showErrorModal('Delete Failed', error.message || 'An error occurred while deleting the video.');
            }
        }
    );
}
...
async function restoreVideoFromReport(videoId, reportId) {
    // Validate videoId exists (same validation as Dart: check for null/undefined/empty)
    if (!videoId || videoId === 'undefined' || videoId === 'null' || videoId === '') {
        showErrorModal('Restore Failed', 'Video ID not found. Cannot restore unknown video.');
        return;
    }
    ...
    showConfirmModal(
        ...
        async () => {
            ...
            try {
                ...
                if (!videoDoc.exists) {
                    hideLoading();
                    showErrorModal('Restore Failed', 'Video not found in database.');
                    return;
                }
                
                // Restore video by removing deleted flags (matching Dart implementation)
                await db.collection('videos').doc(videoId).update({
                    isDeleted: false,
                    restoredAt: firebase.firestore.FieldValue.serverTimestamp(),
                    restoredBy: auth.currentUser.email
                ...
                closeReportDetail();
                hideLoading();
                showSuccessModal('Video Restored', 'Video restored successfully');
            } catch (error) {
                console.error('Restore video error:', error);
                hideLoading();
                showErrorModal('Restore Failed', error.message || 'An error occurred while restoring the video.');
            }
        }
    );
}
...
function closeWarningModal(event) {
    if (event && event.target.id !== 'warningModal') return;
