...
function loadUserManagement() {
    ...
    const loadUsers = () => {
        ...
        if (statusFilter === 'suspended') {
            ...
            // For active users, we need to filter out suspended and deleted users
            // Since Firestore doesn't support multiple != queries, we fetch all and filter client-side
            query = query.where('isSuspended', '==', false).where('isDeleted', '==', false);
        }
        ...
        const unsubscribe = query.onSnapshot((snapshot) => {
            ...
            snapshot.forEach(doc => {
                ...
                // Additional client-side filtering for 'all' and 'active' cases
                if (statusFilter === 'all') {
                    // Show all users regardless of status
                    allUsers.push(userData);
                } else if (statusFilter === 'active') {
                    // Ensure user is truly active (not suspended and not deleted)
                    if (!userData.isSuspended && !userData.isDeleted) {
                        allUsers.push(userData);
                    }
                } else if (statusFilter === 'suspended') {
                    // Only show suspended users (exclude deleted)
                    if (userData.isSuspended && !userData.isDeleted) {
                        allUsers.push(userData);
                    }
                } else if (statusFilter === 'deleted') {
