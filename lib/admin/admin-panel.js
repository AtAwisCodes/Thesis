...
function showConfirmModal(title, message, confirmText, cancelText, onConfirm) {
    // Create confirmation modal that appears ABOVE other modals
    const confirmModalHtml = `
        <div class="modal-overlay" id="confirmModal" onclick="closeConfirmModal(event)" style="z-index: 1100; background: rgba(0, 0, 0, 0.7);">
            <div class="modal" onclick="event.stopPropagation()">
                ...
                <div class="modal-footer">
                    <button class="btn btn-cancel" onclick="closeConfirmModal()">${cancelText}</button>
    ...
    // Append to body instead of modalContainer
...
function closeConfirmModal(event) {
    if (event && event.target.id !== 'confirmModal') return;
...
// ============================================
// ADMIN EMAIL WHITELIST - SECURE SOLUTION
// Only these emails can access admin panel
// Passwords are stored securely in Firebase
// ============================================
...
// Authentication State Observer with Whitelist Check
auth.onAuthStateChanged((user) => {
    if (user) {
        // Double-check: Verify user email is in whitelist
        if (!isAuthorizedAdmin(user.email)) {
            // Unauthorized - sign out immediately
            auth.signOut();
            showError('Access Denied: This account is not authorized for admin access.');
            return;
        }
...
// Login Form Handler with Whitelist + Password Validation
loginForm.addEventListener('submit', async (e) => {
    ...
    try {
        // FIRST CHECK: Is email in whitelist?
        if (!isAuthorizedAdmin(email)) {
            throw new Error('Access Denied: This email is not authorized for admin access.');
        }
        
        // SECOND CHECK: Verify password with Firebase
        await auth.signInWithEmailAndPassword(email, password);
        
        // Both checks passed - access granted
        
    } catch (error) {
        // Security: If somehow logged in but not authorized, sign out
        if (auth.currentUser && !isAuthorizedAdmin(auth.currentUser.email)) {
            await auth.signOut();
        }
        ...
        return `
            <div class="search-container">
                <div style="display: flex; gap: 12px; margin-bottom: 16px;">
                    ...
                    >
                    <button 
                        onclick="syncAllUsersFromFirebase()" 
...
function renderUserCard(user) {
    ...
    return `
        <div class="user-card" onclick="showUserDetailModal('${user.id}')">
                    ...
                    <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); margin-bottom: 20px;">
                        ...
                        <div style="border-top: 1px solid #eee; padding-top: 16px;">
                            ...
                            <div class="info-row">
                                ...
                                <a href="#" onclick="closeReportDetail(); navigateToUser('${reporterEmail}'); return false;" style="color: #2196f3; text-decoration: none; cursor: pointer;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">
                                    ${reporterEmail} →
                                </a>
                            </div>
                            <div class="info-row">
                                <strong style="width: 140px; display: inline-block; color: #666;">Uploader:</strong>
                                <a href="#" onclick="closeReportDetail(); navigateToUser('${uploaderEmail}'); return false;" style="color: #2196f3; text-decoration: none; cursor: pointer;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">
                                    ${uploaderEmail} →
                                </a>
                            </div>
                            <div class="info-row">
...
async function showUserDetailModal(userId) {
    ...
    try {
        ...
        if (!userDoc.exists) {
            hideLoading();
            showErrorModal('User Not Found', 'The user could not be found.');
            return;
        }
        ...
        const createdDate = userData.createdAt ? new Date(userData.createdAt.toDate()).toLocaleString('en-US', {
            month: 'long',
            day: 'numeric',
            year: 'numeric',
            hour: '2-digit',
        ...
        const deletedVideos = videosSnapshot.docs.filter(doc => doc.data().isDeleted).length;
        const activeVideos = totalVideos - deletedVideos;
        
        // Get user's reports
        const reportsSnapshot = await db.collection('video_reports')
            ...
            <div class="modal-body">
                ...
                <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); margin-bottom: 20px;">
                    <div style="display: flex; gap: 20px; align-items: center; margin-bottom: 20px;">
                        <div style="width: 80px; height: 80px; border-radius: 50%; background: rgba(103, 58, 183, 0.1); display: flex; align-items: center; justify-content: center; font-size: 32px; font-weight: bold; color: #673ab7; flex-shrink: 0;">
                            ${userData.avatarUrl ? `<img src="${userData.avatarUrl}" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover;">` : initial}
                        </div>
                        <div style="flex: 1;">
                            <div style="font-size: 24px; font-weight: bold; margin-bottom: 8px; display: flex; align-items: center; gap: 12px;">
                                ${userData.displayName || 'Unknown User'}
                                ${isSuspended ? '<span style="padding: 4px 12px; border-radius: 16px; font-size: 12px; font-weight: bold; color: white; background: #ff9800;">SUSPENDED</span>' : ''}
                                ${isDeleted ? '<span style="padding: 4px 12px; border-radius: 16px; font-size: 12px; font-weight: bold; color: white; background: #f44336;">DELETED</span>' : ''}
                            </div>
                            <div style="color: #666; font-size: 16px; margin-bottom: 4px;">${userData.email || 'No email'}</div>
                            <div style="color: #999; font-size: 14px;">Member since ${createdDate}</div>
                        </div>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 20px;">
                        <div style="background: #e3f2fd; padding: 16px; border-radius: 8px; text-align: center;">
                            <div style="font-size: 28px; font-weight: bold; color: #1976d2; margin-bottom: 4px;">${totalVideos}</div>
                            <div style="font-size: 12px; color: #666;">Total Videos</div>
                        </div>
                        <div style="background: #e8f5e9; padding: 16px; border-radius: 8px; text-align: center;">
                            <div style="font-size: 28px; font-weight: bold; color: #388e3c; margin-bottom: 4px;">${activeVideos}</div>
                            <div style="font-size: 12px; color: #666;">Active Videos</div>
                        </div>
                        <div style="background: #ffebee; padding: 16px; border-radius: 8px; text-align: center;">
                            <div style="font-size: 28px; font-weight: bold; color: #d32f2f; margin-bottom: 4px;">${totalReports}</div>
                            <div style="font-size: 12px; color: #666;">Reports Received</div>
                        </div>
                    </div>
                    ...
                    <div style="border-top: 1px solid #eee; padding-top: 16px;">
                        <div style="margin-bottom: 12px;">
                            <strong style="color: #666; display: inline-block; width: 120px;">User ID:</strong>
                            <span style="font-family: monospace; font-size: 12px; color: #999;">${userId}</span>
                        </div>
                        ${userData.bio ? `
                            <div style="margin-bottom: 12px;">
                                <strong style="color: #666; display: block; margin-bottom: 8px;">Bio:</strong>
                                <div style="padding: 12px; background: #f5f5f5; border-radius: 8px; line-height: 1.6;">${userData.bio}</div>
                            </div>
                        ` : ''}
                    </div>
                </div>
                ...
                ${(isSuspended || isDeleted) ? `
                    <div style="background: ${isSuspended ? '#fff3e0' : '#ffebee'}; padding: 16px; border-radius: 12px; border: 1px solid ${isSuspended ? '#ffe0b2' : '#ffcdd2'}; margin-bottom: 20px;">
                        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="${isSuspended ? '#f57c00' : '#d32f2f'}">
                                <path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/>
                            </svg>
                            <strong style="color: ${isSuspended ? '#e65100' : '#b71c1c'}; font-size: 16px;">${isSuspended ? 'Suspension Information' : 'Deletion Information'}</strong>
                        </div>
                        ${userData.suspensionReason ? `
                            <div style="margin-bottom: 8px;">
                                <strong style="color: #666; font-size: 14px;">Reason:</strong>
                                <div style="margin-top: 4px; color: #333;">${userData.suspensionReason}</div>
                            </div>
                        ` : ''}
                        ${userData.deletionReason ? `
                            <div style="margin-bottom: 8px;">
                                <strong style="color: #666; font-size: 14px;">Reason:</strong>
                                <div style="margin-top: 4px; color: #333;">${userData.deletionReason}</div>
                            </div>
                        ` : ''}
                        ${userData.suspendedBy ? `
                            <div style="font-size: 12px; color: #666; margin-top: 8px;">Actioned by: ${userData.suspendedBy}</div>
                        ` : ''}
                        ${userData.deletedBy ? `
                            <div style="font-size: 12px; color: #666; margin-top: 8px;">Actioned by: ${userData.deletedBy}</div>
                        ` : ''}
                        ${userData.suspensionEndDate ? `
                            <div style="font-size: 12px; color: #666; margin-top: 4px;">Ends: ${new Date(userData.suspensionEndDate.toDate()).toLocaleString()}</div>
                        ` : ''}
                    </div>
                ` : ''}
                ...
                <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 12px;">
                        ${!isDeleted && !isSuspended ? `
                            <button class="btn btn-warning" onclick="closeModal(); suspendUser('${userId}', '${userData.displayName}', '${userData.email}')">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                        ...
                        ${isSuspended && !isDeleted ? `
                            <button class="btn btn-success" onclick="closeModal(); unsuspendUser('${userId}', '${userData.displayName}')">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z"/>
                                </svg>
                                Unsuspend User
                        ...
                        ${!isDeleted ? `
                            <button class="btn btn-danger" onclick="closeModal(); deleteUser('${userId}', '${userData.displayName}', '${userData.email}')">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/>
                                </svg>
                                Delete Account
                        ...
                        ${isDeleted ? `
                            <button class="btn btn-primary" onclick="closeModal(); restoreUser('${userId}', '${userData.displayName}')">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M13 3c-4.97 0-9 4.03-9 9H1l3.89 3.89.07.14L9 12H6c0-3.87 3.13-7 7-7s7 3.13 7 7-3.13 7-7 7c-1.93 0-3.68-.79-4.94-2.06l-1.42 1.42C8.27 19.99 10.51 21 13 21c4.97 0 9-4.03 9-9s-4.03-9-9-9z"/>
                                </svg>
                                Restore Account
        ...
        hideLoading();
        showErrorModal('Error Loading User', error.message);
    }
}

// Navigate to specific user
async function navigateToUser(userEmail) {
    showLoading();
    try {
        ...
        if (!usersSnapshot.empty) {
            ...
            hideLoading();
            // Open the user detail modal directly
            showUserDetailModal(userId);
        } else {
            hideLoading();
            showErrorModal('User Not Found', `No user found with email: ${userEmail}`);
        }
    } catch (error) {
        hideLoading();
        showErrorModal('Error', `Failed to find user: ${error.message}`);
    }
}

// Sync all users from Firebase Auth to users collection
async function syncAllUsersFromFirebase() {
    // Show confirmation dialog first
    const confirmed = await new Promise((resolve) => {
        showConfirmModal(
            'Sync All Users',
            'This will scan the "count" collection and create user management documents for any missing users. Continue?',
            'Sync Users',
            'Cancel',
            () => resolve(true)
        );
    ...
    try {
        ...
        const countSnapshot = await db.collection('count').get();
        
        console.log(`Found ${countSnapshot.size} documents in 'count' collection`);
        ...
        for (const countDoc of countSnapshot.docs) {
            try {
                const userId = countDoc.id;
                ...
                if (!userDoc.exists) {
                    ...
                    const firstName = countData.first_name || '';
                    const lastName = countData.last_name || '';
                    ...
                    await db.collection('users').doc(userId).set({
                        ...
                        createdAt: countData.created_at || firebase.firestore.FieldValue.serverTimestamp(),
                        isSuspended: false,
                        isDeleted: false
                    });
                    
                    console.log(`✅ Synced user: ${displayName} (${countData.email})`);
                    syncedCount++;
                } else {
                    console.log(`⏭️ Skipped existing user: ${userId}`);
                    skippedCount++;
                }
            } catch (error) {
                console.error(`❌ Error syncing user ${countDoc.id}:`, error);
                errors.push(`${countDoc.id}: ${error.message}`);
                errorCount++;
            }
        }
        ...
        const message = `
            <div style="text-align: left;">
                ...
                <p>✅ Added: <strong>${syncedCount}</strong> users</p>
                <p>ℹ️ Already existed: <strong>${skippedCount}</strong> users</p>
                <p>📊 Total in 'count' collection: <strong>${countSnapshot.size}</strong></p>
                ${errorCount > 0 ? `<p style="color: #ff9800;">⚠️ Errors: <strong>${errorCount}</strong> users</p>` : ''}
                ${errors.length > 0 ? `<details style="margin-top: 8px;"><summary>Error Details</summary><pre style="font-size: 11px; max-height: 200px; overflow-y: auto;">${errors.join('\n')}</pre></details>` : ''}
                <p style="margin-top: 12px; padding: 8px; background: #e3f2fd; border-radius: 4px; font-size: 12px;">
                    <strong>Note:</strong> Only users in the 'count' collection are synced. If Firebase Auth users are missing, they need to log in at least once to be added.
                </p>
            </div>
        `;
        
        showSuccessModal('User Sync Complete', message);
        
        // Reload user management to show new users
        ...
        showErrorModal('Sync Failed', `Error syncing users: ${error.message}<br><br>Check console for details.`);
    }
}
...
window.syncAllUsersFromFirebase = syncAllUsersFromFirebase;
