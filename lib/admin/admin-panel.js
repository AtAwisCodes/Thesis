...
async function updateReportStatus(reportId, newStatus) {
    if (!confirm(`Update report status to "${newStatus}"?`)) return;
...
async function deleteVideoFromReport(videoId, reportId) {
    if (!confirm('This will hide the video from public view. You can restore it later if needed.\n\nProceed with deletion?')) return;
    ...
    try {
        ...
        closeReportDetail();
        alert('Video deleted successfully. Can be restored if needed.');
    } catch (error) {
...
async function restoreVideoFromReport(videoId, reportId) {
    if (!confirm('This will make the video visible again to all users.\n\nProceed with restoration?')) return;
    ...
    try {
        ...
        alert('Error restoring video: ' + error.message);
    }
    hideLoading();
}

async function sendWarningToUploader(uploaderId, reportId, videoTitle) {
    const message = prompt('Enter a custom warning message to send to the video uploader:', 'Your content has been reported for violating community guidelines.');
    
    if (!message) return;
    
    showLoading();
    try {
        await db.collection('count').doc(uploaderId).collection('notifications').add({
            type: 'warning',
